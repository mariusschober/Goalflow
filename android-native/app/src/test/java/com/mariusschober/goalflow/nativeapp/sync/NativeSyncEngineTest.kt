...
class NativeSyncEngineTest {
        ...
        val transport = NativeSyncTransport { path, _, _, body ->
            when {
                path == "/api/v1/sync/push" -> {
                    ...
                    val mutation = request.getJSONArray("mutations").getJSONObject(0)
    ...
    private fun acceptedResult(mutation: JSONObject, serverVersion: Long): JSONObject = JSONObject()
        .put("mutationId", mutation.getString("mutationId"))
        .put("accepted", true)
        .put("serverVersion", serverVersion)
        .put("record", JSONObject()
            .put("entity_type", mutation.getString("entityType"))
            .put("entity_id", mutation.getString("entityId"))
            .put("version", mutation.getLong("version"))
            .put("server_version", serverVersion)
            .put("payload", mutation.get("payload"))
            .put("updated_at", mutation.getString("updatedAt"))
            .put("deleted_at", mutation.opt("deletedAt")))
