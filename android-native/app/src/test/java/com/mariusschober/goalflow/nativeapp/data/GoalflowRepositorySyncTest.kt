...
class GoalflowRepositorySyncTest {
    ...
    fun `accepted creation promotes completion and rejection preserves completion conflict`() = runTest {
        ...
        val create = repository.readySyncMutations().single()
        repository.commitPushResults(
            listOf(create),
            listOf(accepted(create, 10))
        )

        val completion = repository.readySyncMutations().single()
    ...
    @Test
    fun `accepted receipt mismatch cannot consume the outbox`() = runTest {
        repository.createTask(
            title = "Receipt must be exact",
            notes = "",
            schedulePrecision = SchedulePrecision.DAY,
            scheduledFor = LocalDate.now().toString(),
            scheduledTime = null,
        ...
        val mutation = repository.readySyncMutations().single()
        val wrong = accepted(mutation, 9).copy(serverPayload = "{\"id\":\"wrong\"}")

        try {
            repository.commitPushResults(listOf(mutation), listOf(wrong))
            fail("A mismatched durable receipt must be rejected")
        } catch (_: IllegalArgumentException) {
            // The exact pending mutation remains authoritative.
        }
        assertEquals(mutation.mutationId, repository.pendingSyncMutations().single().mutationId)
    }
    ...
    fun `local Room data can never synchronize into a second account`() = runTest {
        ...
        assertEquals("00000000-0000-4000-8000-000000000001", database.localAccountDao().get()?.userId)
    }
