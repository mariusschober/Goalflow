...
final class ServerBreakdownGateway: BreakdownGateway, @unchecked Sendable {
    ...
    init(supabaseUrl: String? = nil, apiOrigin: String? = nil, anonKey: String? = nil) {
        let info = Bundle.main.infoDictionary
        self.supabaseUrl = supabaseUrl ?? (info?["SUPABASE_URL"] as? String) ?? ""
        self.apiOrigin = apiOrigin ?? (info?["API_ORIGIN"] as? String) ?? "https://app.goalflow.com"
    ...
    func suggest(for task: GoalflowTask) async throws -> [BreakdownSuggestion] {
        let token: String
        do {
            token = try await keychain.currentAccessToken(supabaseUrl: supabaseUrl.isEmpty ? nil : supabaseUrl, anonKey: anonKey.isEmpty ? nil : anonKey)
        } catch {
            // Local demo fallback — no auth, return empty (UI can show manual)
            return []
        }
        let base = apiOrigin.hasSuffix("/") ? String(apiOrigin.dropLast()) : apiOrigin
        guard let url = URL(string: "\(base)/api/v1/ai/breakdown") else { throw BreakdownError.invalid }
...
enum BreakdownError: Error, LocalizedError {
    case quotaReached, unavailable, invalid
